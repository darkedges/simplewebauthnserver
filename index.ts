...
import type {
  GenerateAuthenticationOptionsOpts, GenerateRegistrationOptionsOpts, VerifiedAuthenticationResponse, VerifiedRegistrationResponse, VerifyAuthenticationResponseOpts, VerifyRegistrationResponseOpts
...
import type {
  ...
  AuthenticatorDevice, RegistrationResponseJSON
...
const {
  ENABLE_CONFORMANCE,
  ENABLE_HTTPS,
  RP_ID = 'webauthn.connectid.darkedges.com',
} = process.env;
...
if (ENABLE_HTTPS) {
  ...
  const host = '0.0.0.0';
  const port = 8000;
  expectedOrigin = `https://webauthn.connectid.darkedges.com`;
